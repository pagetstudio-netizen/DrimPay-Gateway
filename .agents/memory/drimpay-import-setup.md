...
- **How to apply:** when a DB write fails mysteriously (especially after importing/forking the project into a new environment), connect directly with `pg` using `SUPABASE_DATABASE_URL` (available in `lib/db`'s node_modules) and diff `information_schema.columns` against the Drizzle schema before debugging app code. Both `payment_links.image_url` and `qr_codes.image_url` were missing on the live Supabase DB — check other tables with an `imageUrl`/image-upload feature too if similar bugs recur.

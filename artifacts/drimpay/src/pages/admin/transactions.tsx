    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between">
          <h2 className="text-lg font-bold text-gray-900">Détails Transaction #{tx.id}</h2>
...
export default function AdminTransactions() {
  ...
  const load = async (p = page) => {
    setLoading(true);
    const params = new URLSearchParams({ page: String(p), limit: String(LIMIT), type: filterType, status: filterStatus, countryCode: filterCountry, search });
  ...
  useEffect(() => { load(); }, [filterType, filterStatus, filterCountry]);

  const exportCSV = () => {
    const rows = txs.map(t => [t.id, t.reference, t.type, t.status, t.amount, t.fee, t.currency, t.countryCode, t.operator, t.phone, t.createdAt].join(","));
    const csv = ["ID,Référence,Type,Statut,Montant,Frais,Devise,Pays,Opérateur,Téléphone,Date", ...rows].join("\n");
  ...
  const bigAlerts = txs.filter(t => parseFloat(t.amount) > 60000);
      ...
      <div className="max-w-7xl mx-auto space-y-5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">{total.toLocaleString("fr-FR")} transactions au total</p>
          </div>
          <div className="flex gap-2">
            {bigAlerts.length > 0 && (
              <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700 font-medium">
                <AlertTriangle className="w-4 h-4" /> {bigAlerts.length} transaction(s) &gt;60k XOF
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex flex-wrap gap-3">
            <div className="flex-1 min-w-[200px] flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2">
              ...
              <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); load(1); }} placeholder="Référence, téléphone..." className="bg-transparent text-sm flex-1 outline-none placeholder:text-gray-400" />
            </div>
            {[
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    {["ID", "Marchand", "Type", "Montant", "Frais", "Pays", "Opérateur", "Statut", "Date", ""].map(h => (
                ...
                <tbody>
                  {txs.map(tx => (
                    <tr key={tx.id} className={cn("border-b border-gray-50 hover:bg-gray-50 transition-colors", parseFloat(tx.amount) > 60000 && "bg-red-50/50 hover:bg-red-50")}>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => setSelected(tx)} className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-500 transition-colors"><Eye className="w-3.5 h-3.5" /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>

...
export default function DocPayin() {
    ...
    <DashboardLayout>
      <div className="max-w-4xl mx-auto">
        ...
        <Section title="Initier un Pay-in" icon={Send}>
          ...
          <CodeBlock lang="curl" code={`curl -X POST https://drimpay.com/api/v2/payin/initiate \\
  ...
  "expires_at": "2024-05-07T10:15:00Z"
        ...
        <Section title="Vérifier le statut" icon={SearchCheck}>
          ...
          <CodeBlock lang="curl" code={`curl -X GET https://drimpay.com/api/v2/payin/PAY-1715000000-A1B2C3D4 \\
  -H "Authorization: Bearer dp_live_xxxx"`} />
  ...
  "completed_at": "2024-05-07T10:02:35Z"
        ...
        <Section title="Lister les Pay-ins" icon={List}>
          ...
          <CodeBlock lang="curl" code={`curl "https://drimpay.com/api/v2/payin?page=1&limit=20&status=success" \\
  -H "Authorization: Bearer dp_live_xxxx"`} />
        </Section>
...
app.post("/webhook/drimpay", express.raw({ type: "application/json" }), (req, res) => {
  ...
  try {
    const res = await fetch("https://drimpay.com/api/v2/payin/initiate", {
      method: "POST",
      headers: {

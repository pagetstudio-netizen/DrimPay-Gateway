    ...
    <div className="pt-24 pb-20">
      <div className="container mx-auto px-4 md:px-8">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-2xl mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-secondary border border-border mb-6 text-xs font-medium">{t.countries.badge}</div>
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-6">{t.countries.title}</h1>
          <p className="text-xl text-muted-foreground leading-relaxed">{t.countries.desc}</p>
        </motion.div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
            {(countries ?? []).map((country, i) => (
              ...
              >
                ...
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">{t.countries.operators}</p>
        ...
        <div className="text-center rounded-2xl bg-primary/10 border border-primary/20 p-10">
          <h2 className="text-2xl font-bold mb-4">{t.countries.noCountryTitle}</h2>
          <p className="text-muted-foreground mb-8 max-w-lg mx-auto">{t.countries.noCountryDesc}</p>
          <Link href="/contact"><Button size="lg" className="text-primary-foreground font-semibold">{t.countries.contactUs} <ArrowRight className="ml-2 w-4 h-4" /></Button></Link>
        </div>
      </div>
    </div>
  );
